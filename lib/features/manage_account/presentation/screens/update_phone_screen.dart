...
import '../../../user/presentation/screens/verify_fix_main_number_screen.dart';
...
class UpdatePhoneScreen extends StatelessWidget {
  ...
  UpdatePhoneScreen({Key? key, this.enrollment = false}) : super(key: key);
                  ...
                  if (state is SendPhoneSuccessful) {
                    if (enrollment) {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => VerifyFixMainNumberScreen(
                                    phoneNumber:
  ...
  onSendPhone() {
    return cubit.sendPhoneNumber(EditPhoneRequestModel(
      phoneNumber: _countryCode.text + _phone.text.replaceAll(' ', ''),
    ));
  }
}
